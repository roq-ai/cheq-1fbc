...
import { getNetworkById, updateNetworkById } from 'apiSdk/networks';
...
function NetworkEditPage() {
  ...
  const handleSubmit = async (values: NetworkInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateNetworkById(id, values);
      mutate(updated);
      resetForm();
  ...
  const formik = useFormik<NetworkInterface>({
    initialValues: data,
