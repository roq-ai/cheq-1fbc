...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<NetworkInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/networks` : null),
    () => getNetworks({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Network',
      path: isTenantUser ? `/networks/view/${data?.[0]?.id}` : '/networks',
      entity: 'network',
